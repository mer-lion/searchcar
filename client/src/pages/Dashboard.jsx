...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            placeholder="AI ile ara... Örn: '500bin altı otomatik Golf'"
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-2 bg-purple-600 text-white rounded-lg text-sm hover:bg-purple-700 disabled:opacity-50"
          >
            {searchLoading ? "Aranıyor..." : "AI Ara"}
          </button>
        </div>
        {searchResults && (
          <div className="mt-4">
            <p className="text-xs text-gray-500 mb-2">
              Filtreler: {JSON.stringify(searchResults.filters)} — {searchResults.count} sonuç
            </p>
            <div className="space-y-2">
              {searchResults.results.slice(0, 5).map((l) => (
                <div key={l.id} onClick={() => navigate(`/listings/${l.id}`)}
                  ...
                  <span className="font-medium truncate mr-2">{l.brand} {l.model} {l.year}</span>
                  <span className="whitespace-nowrap">{l.price?.toLocaleString("tr-TR")} TL</span>
                </div>
              ))}
              {searchResults.count === 0 && <p className="text-sm text-gray-500">Sonuç bulunamadı.</p>}
            </div>
          </div>
        )}
      </div>
      ...
      <div>
        <h2 className="text-base md:text-lg font-semibold text-gray-900 mb-3 md:mb-4">En İyi 5 Fırsat</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          ...
          {(!stats?.topOpportunities || stats.topOpportunities.length === 0) && (
            <p className="text-gray-500 text-sm">Henüz analiz edilmiş ilan yok.</p>
          )}
        </div>
      </div>
    </div>
