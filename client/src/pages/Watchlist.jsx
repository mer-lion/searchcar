...
export default function Watchlist() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6">
      <h1 className="text-xl md:text-2xl font-bold text-gray-900">Takip Listesi</h1>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={item.id} className="border-b border-gray-100">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">{item.price_max?.toLocaleString("tr-TR") || "-"}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleRemove(item.id)}
                      className="text-red-600 hover:underline text-xs whitespace-nowrap">Sil</button>
                  </td>
                </tr>
              ))}
            </tbody>
